...
data = np.loadtxt(
    "data.txt" if system == "Mass-Spring" else "RC_data.txt",
    usecols=range(0, 9),
    delimiter=",",
    skiprows=1,
)
...
plt.xlabel("time")
plt.ylabel("value")
plt.title(f"Mass-Spring System Time Evolution {algorithm} {system}")
plt.legend()
plt.grid()
...
plt.xlabel("position")
plt.ylabel("velocity")
plt.title(f"Mass-Spring System Phase Plot {algorithm} {system}")
plt.legend()
plt.grid()
